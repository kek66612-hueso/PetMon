    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        switchTheme.setOnCheckedChangeListener((buttonView, isChecked) -> {
            ...
            if (!userLogin.isEmpty()) {
                // Отправляем в Firestore в фоне (станет в очередь при отсутствии сети)
                FirebaseFirestore.getInstance().collection("Users").document(userLogin)
                        .set(Collections.singletonMap("theme_mode", newTheme), SetOptions.merge());
            } 
            
            // Сразу применяем тему и перезагружаем активити
